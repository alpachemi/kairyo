Kairyo Core version *0.15.1* is now available from:

  <https://download.kairyo.org/kairyo-0.15.1/>
...
A number of changes to the way Kairyo Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lakes per kB (0.00001 RYO). This is to prepare for dropping the minimum transaction fee to 0.1 lakes per kB (0.0001 RYO) in the next release.
